...
<head>
    ...
    <title>Материалы и ПКИ на предприятии. Дашборд начальника цеха "АО "ТЗ "Ревтруд"</title>
        ...
        <ul class="menu">
          ...
          <li style="background: white;">
            <a href="/../directorDashBoard/directorDashBoardDSE.php">
                <img src = '/../svg/gear-svgrepo-com.svg' style = 'width: 50px; margin-left: 20%;'><span style = 'color:black;'>Материалы и ПКИ</span></a>
          </li>
          <li>
...
<div class = 'info-div' id = 'info-div'>Материалы и ПКИ</div>
...
<tr>
 ...
 <tr>
  <?php
  $result = mysqli_query($conn,"SELECT n.*, d.name as name_order from nomenclature_dce n, d_order d where n.ID_order = d.ID "); // запрос на выборку
  $id = 0;
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      $type = 'Материал';
      echo '<tr><td id = "fio-'.$id.'">'.$row['id_name'].'</td>';
      echo '<td>'.$row['name'].'</td>';
      ...
      echo '<td style = "background-color:#F8F8F8;display:none"></td><td style = "background-color:#F8F8F8;display:none"></td><td style  = "display:none;position:fixed;border-radius: 10px;border:1px black solid;width:200px;height:75px;margin-left: 465px;margin-top: -10px;" id = "td-'.$id.'">№ заказа: '.$row['ID_order'].'<br>Наименование заказа: '.$row['name_order'].'</td>';
      $id++;
  }
  ?>
 </tr>
...
<script src="/../js/directorDashBoardDSE.js"></script>
<script>
   google.load("visualization", "1", {packages:["corechart"]});
   google.setOnLoadCallback(drawChart);
   function drawChart() {
    var data = google.visualization.arrayToDataTable([
     ['Наименование ДСЕ', 'Кол-во'],
     <?php
     $result = mysqli_query($conn,"SELECT count,type FROM nomenclature_dce;"); // запрос на выборку
     $pki = 0;
     $matherials = 0;
     while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
     {
         if ($row['type'] == 1)
          ...
          {
              $matherials = $matherials + $row['count'];
          }
     }
     
    echo "['Материалы',".$matherials."],";
    echo "['ПКИ',".$pki."],";
     ?>
    ]);
    var options = {
     title: 'ДСЕ на предприятии',
     is3D: true,
