...
<head>
    ...
    <title>Информация о клиентах. Дашборд начальника цеха "АО "ТЗ "Ревтруд"</title>
        ...
        <ul class="menu">
          ...
          <li style="background: white;">
            <a href="/../directorDashBoard/directorDashBoardClients.php">
              <img src = '/../svg/client-profile-svgrepo-com.svg' style = 'width: 50px; margin-left: 20%;'><span style = 'color:black;'>Информация о клиентах</span></a>
          </li>
          <li>
...
<div class = 'info-div' id = 'info-div'>Информация о клиентах</div>

<div class = 'table-and-diagramm' style = 'margin-right:50px;'>
<div class = 'table'style = 'margin-left:90px;'>
...
<tr>
  ...
  <th>Адрес</th>
  <th>ИНН</th>
  <th style = 'border-top-right-radius: 0px;'>Номер телефона</th>
  </tr>
 <tr>
  <?php
  $id_div_status = 0;
  $id_fiz = 1;
  $count_orders = 1;
  $result = mysqli_query($conn,"SELECT * from client where type_client = 0;"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      echo '<tr style = "height: 150px;"><td style = "border-radius: 0;text-align:center;">'.$id_fiz.'</td>';
      echo '<td>'.$row['FIO'].'<button class = "info-button" onclick = "openAndCloseOrders(this.id);" id ="fiz-'.$id_fiz.'">Заказы</button></td>';
      echo '<td>'.$row['adress'].'</td>';
      echo '<td>'.$row['INN'].'</td>';
      echo '<td style = "border-radius: 0;">'.$row['phone_number'].'</td></tr>';
      $price = $row['price'];
      $price = $price/100;
      echo '<td colspan = "5" style = "text-align:center;display:none;border-radius: 0;" id = "fiz-'.$id_fiz.'-order">';
      $orders = mysqli_query($conn,"SELECT o.name,o.price,o.product_name,o.status from client t, d_order o where t.ID = o.id_client and t.type_client = 0 and t.id = '".$row['ID']."'");
        while($row_1 = $orders->fetch_assoc())
          {
              $price = $row_1['price'];
              $price = $price/100;
              echo $count_orders.'. Наименование заказа: '.$row_1['name'].'<br>';
              echo 'Цена: '.$price.'<br>';
              echo 'Наименование продукта: '.$row_1['product_name'].'<br>';
              echo 'Статус: <div style = "border: 1px black solid;border-radius: 50px;" id = "order-div-status-'.$id_div_status.'">'.$row_1['status'].'</div><br>';
              $count_orders++;
              $id_div_status++;
          }
          $count_orders = 1;
...
<tr>
  ...
  <?php
  $id_ur = 1;
  $result = mysqli_query($conn,"SELECT * from client where type_client = 1"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      echo '<tr style = "height: 150px;"><td style = "border-radius: 0;text-align:center;">'.$id_ur.'</td>';
      echo '<td>'.$row['Ur_name'].'<button class = "info-button" onclick = "openAndCloseOrders(this.id);" id ="ur-'.$id_ur.'">Заказы</button></td>';
      echo '<td>'.$row['adress'].'</td>';
      echo '<td><a target = "_blank" href = "'.$row['RusProfile'].'">'.$row['INN'].'</a></td>';
      echo '<td style = "border-radius: 0;">'.$row['phone_number'].'</td></tr>';
      $price = $row['price'];
      $price = $price/100;
      echo '<td colspan = "5" style = "text-align:center;display:none;border-radius: 0;" id = "ur-'.$id_ur.'-order">';
      $orders = mysqli_query($conn,"SELECT o.name,o.price,o.product_name,o.status from client t, d_order o where t.ID = o.id_client and t.type_client = 1 and t.id = '".$row['ID']."'");
        while($row_1 = $orders->fetch_assoc())
          {
              $price = $row_1['price'];
              $price = $price/100;
  ...
  echo '<input style = "display:none" id = "inputTotal" value = '.$id_div_status.'></input>';
  ?>
</table>
</div>
...
<script src="/../js/directorDashBoardClients.js"></script>
</body>
</html>