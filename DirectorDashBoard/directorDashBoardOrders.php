...
<head>
    ...
    <title>Информация о заказах. Дашборд начальника цеха "АО "ТЗ "Ревтруд"</title>
...
<header class="main-head">
      <nav class="head-nav">
        <ul class="menu">
          <li>
            <a href="/../directorDashBoard/directorDashBoard.php">
                <img src = '/../svg/employee-card-svgrepo-com.svg' style = 'width: 50px; margin-left: 20%;'><span>Информация о сотрудниках</span></a>
          </li>
          <li>
          ...
          <li style="background: white;">
            <a href="/../directorDashBoard/directorDashBoardOrders.php">
                <img src = '/../svg/tables-couple-svgrepo-com.svg' style = 'width: 50px; margin-left: 20%;'><span style ='color:black;'>Заказы и работы</span></a>
          </li>
          <li>
...
<div class = 'info-div' id = 'info-div'>Заказы и работы</div>

<div style = 'display:flex;'>
<div class="post-item">
<div class="item-content" onclick = 'showTableNowOrders();'>
<div class="item-body">
<h3>Общее кол-во текущих заказов: </h3>
<p>
<?php
 $total_count = 0;
  $result = mysqli_query($conn,"SELECT o.ID,o.name,o.price,o.product_name,o.status,t.FIO,t.Ur_name from client t, d_order o where t.ID = o.id_client and o.status = 0;"); // запрос на выборку
...
<a href="#" class="link"><span id = 'now-orders-text'>Показать таблицу текущих заказов</span></a>
</div>
</div>
</div>
...
<div class="item-content" onclick= 'showTableNowWorks();'>
<div class="item-body">
<h3>Общее кол-во текущих работ: </h3>
<p>

<?php
$total_count_current_works = 0;
  $result = mysqli_query($conn,"SELECT t.id,t.name,t.status,e.FIO from current_works t, brigade b, employee e where e.id = b.employeers_id and b.ID = t.ID_brigade and status = 0;"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
    $total_count_current_works++;
  }
  echo $total_count_current_works;
  ?>

</p>
</div>
<div class="item-footer">
<a href="#" class="link"><span id = 'now-works-text'>Показать таблицу текущих работ</span></a>
</div>
</div>
</div>
<div id="columnchart_values" style="width: 600px; height: 225px; margin-top:15px; padding:10px;"></div>
</div>


<div id = 'now-orders' class = 'table' style = 'margin-left:115px;display:none;'>
<table>
<tr>
  <th>№</th>
  ...
  <th>Имя юр.лица</th>
  <th>Статус
    <button class = 'info-button' onclick = 'showTableReadyOrders();'>Выполненные заказы</button>
  </th>
  </tr>
 <tr>
 <?php
  $result = mysqli_query($conn,"SELECT o.ID,o.name,o.price,o.product_name,o.status,t.FIO,t.Ur_name from client t, d_order o where t.ID = o.id_client and o.status = 0 order by o.ID;"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      $price = $row['price'];
      $price = $price/100;
      ...
      echo '<td>'.$row['FIO'].'</td>';
      echo '<td>'.$row['Ur_name'].'</td>';
      echo '<td style = "width:150px;"><div style = "border:1px black solid;border-radius:50px;background-color:#FFFF00">В процессе<div></td>';
  }
  ?>
 </tr>
</table>
...
<div id = 'complete-orders' class = 'table' style = 'margin-left:115px;display:none'>
<table>
<tr>
  <th>№</th>
  <th>Наименование заказа</th>
  ...
  <th>Имя юр.лица</th>
  <th>Статус
    <button class = 'info-button' onclick = 'showTableReadyOrders();'>Текущие заказы</button>
  </th>
  </tr>
 <tr>
 <?php
  $result = mysqli_query($conn,"SELECT o.ID,o.name,o.price,o.product_name,o.status,t.FIO,t.Ur_name from client t, d_order o where t.ID = o.id_client and o.status = 1 order by o.ID;"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      $price = $row['price'];
      $price = $price/100;
      ...
      echo '<td>'.$row['FIO'].'</td>';
      echo '<td>'.$row['Ur_name'].'</td>';
      echo '<td style = "width:150px;"><div style = "border:1px black solid;border-radius:50px;background-color:#00FF00">Выполнен<div></td>';
  }
  ?>
 </tr>
</table>
...
<div id = 'now-works' class = 'table' style = 'margin-left:115px;display:none;'>
<table>
<tr>
  <th>№</th>
  ...
  <th>ФИО работника</th>
  <th>Статус
    <button class = 'info-button' onclick = 'showTableReadyWorks();'>Выполненные работы</button>
  </th>
  </tr>
 <tr>
 <?php
  $result = mysqli_query($conn,"SELECT t.id,t.name,t.status,e.FIO from current_works t, brigade b, employee e where e.id = b.employeers_id and b.ID = t.ID_brigade and status = 0 order by t.id;"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      echo '<tr><td>'.$row['id'].'</td>';
      echo '<td>'.$row['name'].'</td>';
...
<div id = 'complete-works' class = 'table' style = 'margin-left:115px;display:none;'>
<table>
<tr>
  <th>№</th>
  <th>Наименование работы</th>
  <th>ФИО работника</th>
  <th>Статус
    <button class = 'info-button' onclick = 'showTableReadyWorks();'>Текущие работы</button>
  </th>
  </tr>
 <tr>
 <?php
  $result = mysqli_query($conn,"SELECT t.id,t.name,t.status,e.FIO from current_works t, brigade b, employee e where e.id = b.employeers_id and b.ID = t.ID_brigade and status = 1 order by t.id;"); // запрос на выборку
  while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
  {
      echo '<tr><td>'.$row['id'].'</td>';
      echo '<td>'.$row['name'].'</td>';
      echo '<td>'.$row['FIO'].'</td>';
      echo '<td style = "width:150px;"><div style = "border:1px black solid;border-radius:50px;background-color:#00FF00">Выполнена<div></td>';
  }
  ?>
 </tr>
</table>
...
<script src="/../js/directorDashBoardOrders.js"></script>
  ...
  <script type="text/javascript">
    ...
    function drawChart() {
      var data = google.visualization.arrayToDataTable([
        ...
        $thursday = 0;
        $friday = 0;
//SELECT dayname(order_date) FROM d_order WHERE (order_date BETWEEN(CURDATE()-WEEKDAY(CURDATE()))AND CURDATE());
        $result = mysqli_query($conn,"SELECT dayname(order_date) as day FROM d_order WHERE (order_date BETWEEN(CURDATE()-WEEKDAY(CURDATE()))AND CURDATE());"); // запрос на выборку
        while($row = $result->fetch_assoc())// получаем все строки в цикле по одной
        {
            if ($row['day'] == 'Monday')
              {
        ...
        echo '["Пн.",'.$monday.', "#42aaff"],';
        echo '["Вт.",'.$tuesday.', "#42aaff"],';
        echo '["Ср.",'.$wednsday.', "#42aaff"],';
        echo '["Чт.",'.$thursday.', "#42aaff"],';
        echo '["Пт.",'.$friday.', "#42aaff"],';
        ?>
      ]);
      ...
      var options = {
        title: "Количество заказов за неделю",
        width: 600,
        height: 225,
        bar: {groupWidth: "65%"},
